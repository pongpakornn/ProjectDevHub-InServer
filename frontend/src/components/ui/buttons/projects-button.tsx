...
/**
 * Projects Button
 * Original design by vinodjangid07 (Uiverse.io)
 * ไอคอนหมุน 2 ทิศทางตอน hover (background หมุนทวนเข็ม, foreground หมุนตามเข็ม)
 */
  ...
  return (
    <>
      <button
        type={type}
        onClick={onClick}
        className={`projects-button ${className}`}
      >
        ...
        {/* 🟢 สามารถใช้ class ของ Tailwind ร่วมด้วยเพื่อความยืดหยุ่นได้ครับ */}
        <span className="text font-bold text-slate-50">{label}</span>
      </button>
      ...
      <style jsx>{`
        .projects-button {
          ...
          border-radius: 12px;
          cursor: pointer;
          background: linear-gradient(120deg, rgb(52, 199, 130), rgb(21, 128, 80));
          transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);
          box-shadow: 0px 10px 15px -3px rgba(16, 185, 129, 0.3);
        }
        .icon-container {
          display: flex;
          align-items: center;
        ...
        /* 🟢 จุดปรับแก้ Font ให้เป็นตัวหนา และอ่านง่ายขึ้น */
        .text {
          color: #ffffff;
          font-weight: 700; /* Font Bold */
          font-size: 15px;
          letter-spacing: 0.025em; /* เพิ่มระยะห่างระหว่างอักษรเล็กน้อย */
          text-shadow: 0 1px 2px rgba(0, 0, 0, 0.15); /* ช่วยให้ข้อความลอยเด่น */
        }

        .projects-button:hover .svg-fg {
          animation: clockwise 2s linear infinite;
        }
        .projects-button:hover .svg-bg {
        ...
        .projects-button:hover {
          background-color: rgb(22, 163, 96);
          transform: translateY(-2px) scale(1.02);
          box-shadow: 0px 14px 20px -3px rgba(16, 185, 129, 0.45);
        }

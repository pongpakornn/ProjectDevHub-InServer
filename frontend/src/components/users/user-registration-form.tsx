...
export default function UserRegistrationForm({
  ...
  useBodyScrollLock(isPermissionModalOpen);
  ...
  const totalGranted = formData.Permissions.reduce(
    (acc, p) =>
      acc +
      (p.CanView ? 1 : 0) +
      (p.CanAdd ? 1 : 0) +
  ...
  const maxPossiblePermissions = systemList.length * 6;
  ...
  return (
    <div className="bg-white border border-slate-200/90 rounded-2xl shadow-lg overflow-hidden animate-in fade-in slide-in-from-top-4 duration-300">
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-6">
        ...
        {/* Section 2: Permission Matrix Setting (เปิดเป็นหน้าต่างเด้ง แทนการแทรกตารางเข้ามาในหน้านี้) */}
        <div className="space-y-3 pt-2">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 bg-slate-900 border border-slate-800 rounded-2xl p-4 text-white shadow-md">
              ...
              <div className="min-w-0">
                <h4 className="text-xs font-bold tracking-tight text-slate-100 flex items-center gap-2 truncate">
                  ...
                  <span className="font-mono text-[10px] text-indigo-300 bg-indigo-950/80 px-2.5 py-0.5 rounded-full border border-indigo-700/60 shadow-xs shrink-0">
                    {totalGranted}/{maxPossiblePermissions} สิทธิ์
                  </span>
                </h4>
                <p className="text-[11px] text-slate-400 truncate">
                  คลิกปุ่มด้านขวาเพื่อเปิดหน้าต่างกำหนดสิทธิ์แบบเต็มจอ มองเห็นภาพรวมทุกโมดูลได้ครบถ้วน
                </p>
              </div>
            </div>
            ...
            <button
              ...
              className="group shrink-0 inline-flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] cursor-pointer shadow-md shadow-indigo-500/20"
            >
              <Settings2 className="w-3.5 h-3.5 transition-transform duration-200 group-hover:rotate-45" />
              กำหนดสิทธิ์การใช้งาน
            </button>
          </div>
        </div>

        {/* Permission Matrix Modal — เต็มจอ เพื่อให้มองเห็นภาพรวมระบบได้ครบทุกโมดูล */}
        {isPermissionModalOpen && (
          <Portal>
            ...
            >
              <div
                className="bg-white border border-slate-200/90 rounded-2xl shadow-2xl w-full max-w-6xl max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200"
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-base font-bold text-slate-900">
                        กำหนดสิทธิ์การใช้งานแต่ละโมดูล (Module-Level Access Rights)
                      </h3>
                      <p className="text-xs text-slate-500 font-medium">
                        {formData.FullName || "สมาชิกใหม่"} • ปรับเปลี่ยนติ๊กเลือกสิทธิ์ในตารางด้านล่างได้ทันที
                      </p>
                    </div>
                  </div>
                  <ActionCloseButton onClick={() => setIsPermissionModalOpen(false)} title="ปิดหน้าต่าง" />
                </div>
